...
import { normalizePlantAnalyzerResponse } from '../../lib/normalizeAnalyzerResponse';
import {
...
async function handleAnalyzerResponse(
  ...
  try {
    parsed = normalizePlantAnalyzerResponse(JSON.parse(text) as PlantAnalyzerResponse);
  } catch {
    if (!res.ok) {
